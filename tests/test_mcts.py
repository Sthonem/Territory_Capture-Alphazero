...
from src.encoding import ACTION_SPACE_SIZE
...
class DummyModel(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        batch_size = x.size(0)
        logits = torch.zeros((batch_size, ACTION_SPACE_SIZE), dtype=torch.float32)
...
class TestMCTS(unittest.TestCase):
    ...
    def test_search_returns_legal_move_and_policy_distribution(self) -> None:
        ...
        self.assertEqual(len(policy), ACTION_SPACE_SIZE)
