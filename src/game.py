...
class GameResult:
    ...
    capture_scores: Dict[str, int]
    tiebreak_scores: Dict[str, int]


class TerritoryCaptureGame:
    """A simple game environment that is easy to extend for future AI work."""

    def __init__(self, board_size: int = 6, stones_per_player: int = 10) -> None:
    ...
    def reset(self) -> None:
        ...
        self.captured_by = {PLAYER_X: 0, PLAYER_O: 0}
    ...
    def clone(self) -> "TerritoryCaptureGame":
        ...
        cloned_game.captured_by = dict(self.captured_by)
    ...
    def _apply_captures(self) -> List[Position]:
        """Remove captured stones and track capture bonuses by player."""

        capture_result = find_captured_stones(self.board)
        for row, col in capture_result.positions:
            self.board[row][col] = EMPTY
        self.captured_by[PLAYER_X] += capture_result.captured_counts[PLAYER_O]
        self.captured_by[PLAYER_O] += capture_result.captured_counts[PLAYER_X]
        return capture_result.positions
    ...
    def get_capture_scores(self) -> Dict[str, int]:
        """Return the running capture bonus totals."""

        return dict(self.captured_by)

    def get_total_scores(self) -> Dict[str, int]:
        """Return total points from territory plus capture bonus."""

        return calculate_total_scores(self.get_territory_scores(), self.captured_by)

    def get_winner(self) -> Optional[str]:
        """Return the winning player after total scoring and tie-break."""
    ...
    def get_result(self) -> GameResult:
        ...
        territory_result = self.evaluate_territory()
        total_scores = calculate_total_scores(territory_result.scores, self.captured_by)
        tiebreak_scores = evaluate_tiebreaker(
            territory_result.territory_map,
            self.board_size,
        )
        return GameResult(
            scores=total_scores,
            ...
            territory_scores=dict(territory_result.scores),
            capture_scores=dict(self.captured_by),
