...
def _save_winrate_plot(running_winrate: List[float], path: str | Path) -> None:
    ...
    try:
        ...
        # Keep evaluation usable even when local matplotlib binaries are unavailable.
        destination.write_bytes(_placeholder_png_bytes())


def _placeholder_png_bytes() -> bytes:
    """Return a tiny valid PNG used as a fallback when plotting is unavailable."""
