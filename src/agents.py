...
class MinimaxAgent:
    ...
    def _evaluate_position(self, game: TerritoryCaptureGame, root_player: str) -> float:
        ...
        if game.is_terminal():
            total = game.get_total_scores()
            return (total[root_player] - total[opponent]) * 10.0

        territory_scores = game.get_territory_scores()
        territory_diff = territory_scores[root_player] - territory_scores[opponent]

        capture_scores = game.get_capture_scores()
        capture_diff = capture_scores[root_player] - capture_scores[opponent]
        ...
        mobility_bonus = len(game.get_legal_actions()) / float(total_cells)
