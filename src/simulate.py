...
def run_simulation(
    ...
    for _ in range(num_games):
        ...
        scores = game.get_territory_scores()
...
def format_summary(summary: SimulationSummary) -> str:
    ...
    return "\n".join(
        [
            ...
            f"Average X territory score: {summary.average_x_score:.2f}",
            f"Average O territory score: {summary.average_o_score:.2f}",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--x-agent", default="random", choices=["random", "heuristic"])
    parser.add_argument("--o-agent", default="random", choices=["random", "heuristic"])
